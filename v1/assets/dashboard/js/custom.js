...
$(document).on('change', '.notification_ajax', function () {
    let checker = $(this).prop('checked');
    if(checker){
        $.get("mute_notification?action=unmute", function(data){
            // console.log(data);
        });
    }else{
        $.get("mute_notification?action=mute", function(data){
            // console.log(data);
        });
    }
});
...
$(document).ready(function() {
    ...
    setInterval(function(){
        ...
        if ($(".check_messages").length > 0) {
            $.get("check_status.php", function (data, status) {
                if (parseFloat(data) > 0) {
                    $(".check_messages").css('display','inline-block');
                    $(".check_messages").html(data);
                } else {
                    $(".check_messages").hide();
                }
            });
    ...
    setInterval(function(){
        ...
        if ($(".check_notification").length > 0 ) {
            $.get("check_notification.php", function (data, status) {
                if (parseFloat(data) > 0) {
                    $(".check_notification").show();
                    $(".check_notification").html(data);
                } else {
                    $(".check_notification").hide();
                }
            });
        }

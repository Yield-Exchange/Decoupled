...
if (AuthModel::isLoggedIn()){
    ...
    if ( isset($_GET['id']) && isset($_GET['c_id'])) {
        ...
        Core::activityLog("Bank Pending Deposits -> Action Chat");

        $contract_data = db::getrecord("SELECT c.*,dr.user_id,dr.currency FROM deposits c, offers o, invited i, depositor_requests dr WHERE c.offer_id = o.id AND i.id = o.invitation_id AND dr.id = i.depositor_request_id AND i.invited_user_id = '$user_id'AND c.id='$c_id' AND c.status NOT IN('COMPLETED')");
...
?>
		...
		<div class="content-wrapper">
			...
			<div class="content">
                <h4>Messages</h4>
                <div class="row">
                    <div class="col-md-3"><strong>Institution: </strong><?php echo Core::render($depositor_data['name']);?></div>

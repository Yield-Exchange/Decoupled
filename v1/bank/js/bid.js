$(document).ready(function() {
    ...
    let dateOfDepositWithUserTimezone_copy = dateOfDepositWithUserTimezone;
    let timeNow = (nextHours+1+dateOfDepositWithUserTimezone.hours())+":00";
    ...
    $('.datetimepicker').datetimepicker({
        ...
        onChangeDateTime:function(dp,$input) {
            if(new Date($input.val()).getDate() === dateOfDepositWithUserTimezone.date()) {
                this.setOptions({
                    minTime: timeNow
                });
            }else if ($input.val() !== ""){
            ...
            let dateOfDepositPlusNextHours = moment(dateOfDepositWithUserTimezone_copy.format("YYYY/MM/DD")+" "+timeNow,'YYYY/MM/DD HH:mm'); // this is logically added here DO NOT REMOVE!
            if( dateOfDepositPlusNextHours <= moment($input.val(),'YYYY/MM/DD HH:mm') ) {
                // TODO
            }else if ($input.val() !== ""){
                $input.val("");
            }
...
function isValidURL(u){
    ...
    let res = u.match(/(http(s)?:\/\/.)?(www\.)?[-a-zA-Z0-9@:%._\+~#=]{2,256}\.[a-z]{2,6}\b([-a-zA-Z0-9@:%_\+.~#?&//=]*)/g);
    return (res !== null)
    // if(!elm){
    ...
    $('.file').change(function(){
        ...
        if (size > 25){
            ...
            if (!validateFileType(f)){
                swal("Please upload a disclosure statement in a valid format: jpeg, gif, png, pdf and word documents");
                removeFile($(this));
            }
        }
    });
...
function validateFileType(file){
    ...
    switch (true) {
        ...
        case fileType.startsWith('application/vnd.openxmlformats-officedocument'):

<template>
    <div style="display: flex; flex-direction: column;gap: 5px; height: 44px;">
        <label>
            <input type="file" class="custom-file-input" @change="handleFileChange">
            <div
                style="width: 100%; height: 40px;  padding: 10px 14px; background: #FDFDFD; border-radius: 999px; overflow: hidden; margin-top: 250px; border: 0.50px #D9D9D9 solid; justify-content: flex-start; align-items: center; gap: 8px; display: inline-flex; margin-top: -32px;cursor: pointer">
        ...
        <span style="color: #5063F4;width: 100%;margin-bottom: 15px;">{{ file ? (file.name.length <= 15) ? file.name :
                `${file.name.slice(0, 16)}...` : '' }}</span><br>
                &nbsp;&nbsp;<br>
                &nbsp;&nbsp;
    </div>
</template>

<template>
    <b-modal :size="size" v-model="visible" hide-footer hide-header centered id="twobtnactionx"
        :no-close-on-backdrop="true" style="width: max-content !important; background-color: white !important;">
        <b-container class="bv-example-row" style="width: 100% !important;padding: 0px !important;">
            ...
            <div class="d-flex w-100">
                <div class="mx-3">
                    ...
                    <div>
                        <p style="color: black; font-size: medium; font-weight: 600;"> {{offer?.c_g?.name}}</p>
                    </div>
                    <!-- <hr class="w-50" /> -->
                    <template v-if="offer?.hasCounters && offer?.requireAction">
                        ...
                        <p class="strike-through">
                            {{ offer?.offer_interest_rate.toFixed(2) }}%</p>
                        <div class="rate-type-offer" style="">
                            <!-- {{ rateTypeCheck(rate_type, rate_operator, variable_rate_value) }} -->
                            {{ rateTypeCheck(offer?.rate_type, offer?.rate_operator, offer?.variable_rate_value) }}
                        </div>
                    </template>
                    <template v-else>
                        ...
                        <p class="main-rate">
                            {{ offer?.offer_interest_rate.toFixed(2) }}%
                        </p>
                        <div class="rate-type-offer" style="">
                            {{ rateTypeCheck(offer?.rate_type, offer?.rate_operator, offer?.variable_rate_value) }}

                            <!-- {{ offer?.rate_type, offer?.rate_operator, offer?.variable_rate_value }} -->
                        </div>
                    </template>
                </div>
                </div>
                <div>   
                    <b-row class="mt-3" style="width:100%;padding: 0px !important;">
...
<style scoped>
#twobtnactionx___BV_modal_body_ .modal-body{
...
import ShowRateCard from "../../repo-marketplace/shared/ShowRateCard.vue";
import { rateTypeCheck } from "../../../utils/commonUtils";
export default {
    components: {
    ...
    props: ['title', 'message', 'showm', 'size', 'btnOneText', 'btnTwoText', 'titlespan', 'icon', 'offer'],
    computed: {
        visible() {
            return this.showm;
        }

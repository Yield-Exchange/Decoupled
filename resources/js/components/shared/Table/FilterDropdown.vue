<template>
    ...
    <div class="btn-group">
        ...
        <div class="dropdown-menu" ref="myScrollDown" :class="{ 'd-flex': show_dropdown }">
            ...
            <!-- </div> -->
            <!-- Subscription Limit -->
            <CustomDropdownToggle v-if="filterType == 'campaign' || filterType == 'draft-campaign'"
                title="Subscription Limit" toggleFor="subscriptioncaret" target="acc-sub-limit"
                @toggleCarret="toggleCaret" :carretState="subscriptioncaret">
                <div class="d-flex justify-content-between w-100 gap-3">
                    <CustomFilterinput type="number" v-model="min_subscription" :max="max_subscription"
                        placeholder="1,000,000" label="Min Subscription">
                    </CustomFilterinput>
                    <CustomFilterinput type="number" v-model="max_subscription" :min="min_subscription"
                        placeholder="2,0000,000" label="Max Subscription">
                    </CustomFilterinput>
                </div>
            </CustomDropdownToggle>

            <!-- Rate Limit -->
            <CustomDropdownToggle v-if="filterType == 'products' || filterType == 'featured-products'" title="Rate (%)"
                toggleFor="ratecaret" target="acc-rate" @toggleCarret="toggleCaret" :carretState="ratecaret">
                <div class="d-flex justify-content-between w-100 gap-3">
            ...
            <!-- Dates -->
            <CustomDropdownToggle v-if="filterType == 'campaign' || filterType == 'draft-campaign'" title="Expiry Range"
                toggleFor="expirycaret" target="acc-exp-range" @toggleCarret="toggleCaret" :carretState="expirycaret">
                <div class="d-flex justify-content-between w-100 gap-3">
                    <div class="d-flex flex-column">
                        <p
                            style="color: var(--gray-700, #344054);font-family: Montserrat;font-size: 14px;font-style: normal;font-weight: 400;line-height: 150%; /* 21px */">
                            Min Date</p>
                        <input class="rounded-input" type="date" :max="max_date" v-model="min_expiry">
                    </div>
                    ...
                    <div class="d-flex flex-column">
                        <p
                            style="color: var(--gray-700, #344054);font-family: Montserrat;font-size: 14px;font-style: normal;font-weight: 400;line-height: 150%; /* 21px */">
                            Max Date</p>
                        <input class="rounded-input" type="date" v-model="max_expiry" :min="min_date">
                    </div>
                </div>
            ...
            <CustomDropdownToggle v-if="filterType == 'featured-products'" title="Financial Organizations"
            ...
            <CustomDropdownToggle v-if="filterType == 'products'" title="Financial Organizations"
                toggleFor="fiorganizationcaret" target="acc-fiorganization" @toggleCarret="toggleCaret"
                :carretState="fiorganizationcaret">
                <div class="">
                    <multiselect v-model="fiorganization" :options="options" :multiple="true" :taggable="true">
...
<script>
    ...
    // campaign ,products
    export default {
        props: ['provinces', 'showFilters', 'termLengthType', 'products', 'noclicks', 'industries', 'filterType', 'fiorganizations', 'itemToRemove', 'filtered_item'],
        components: {
            CustomFilterinput,
            Multiselect,
            CustomDropdownToggle,
        ...
        methods: {
            checkFilters() {
                if ((this.max_term == '' && this.min_term == '') && (this.min_clicks == '' && this.max_clicks == '')
                    && (this.min_subscription == '' && this.max_subscription == '') && (this.min_expiry == '' && this.max_expiry == '')
                    && (this.min_rate == '' && this.max_rate == '') && (this.min_lockperiod == '' && this.max_lockperiod == '') && this.productType.length == 0 && this.fiorganization == 0) {
            ...
            toggleCaret(caretType) {
                ...
                if (caretType === 'industrycaret') {
                    this.industrycaret = !this.industrycaret;
                }
            ...
            submitFilter() {
                if (this.filterType == "campaign") {
                    this.filtered_data = `clicks=${this.min_clicks ? this.min_clicks : 0},${this.max_clicks ? this.max_clicks : 0}&subscription=${this.min_subscription ? this.min_subscription : 0},${this.max_subscription ? this.max_subscription : 0}&termLengthType=${this.selectedTermLengthType}&termLength=${this.min_term ? this.min_term : 0},${this.max_term ? this.max_term : 0}&expiry=${this.min_expiry},${this.max_expiry}`
                    this.$emit('apply_filters', this.filtered_data)
                    // console.log(this.filtered_data)
                } else if (this.filterType == "draft-campaign") {
                    this.filtered_data = `subscription=${this.min_subscription ? this.min_subscription : 0},${this.max_subscription ? this.max_subscription : 0}&termLengthType=${this.selectedTermLengthType}&termLength=${this.min_term ? this.min_term : 0},${this.max_term ? this.max_term : 0}&expiry=${this.min_expiry},${this.max_expiry}`
                    this.$emit('apply_filters', this.filtered_data)
                    // console.log(this.filtered_data)
                }

                else if (this.filterType == "products") {
                    this.filtered_data = `&rate=${this.min_rate ? this.min_rate : 0},${this.max_rate ? this.max_rate : 0}&lockoutPeriod=${this.min_lockperiod ? this.min_lockperiod : 0},${this.max_lockperiod ? this.max_lockperiod : 0}&termLength=${this.min_term ? this.min_term : 0},${this.max_term ? this.max_term : 0}&termLengthType=${this.selectedTermLengthType}`
                ...
                else if (this.filterType == "all-products") {
                    this.filtered_data = `lockoutPeriod=${this.min_lockperiod ? this.min_lockperiod : 0},${this.max_lockperiod ? this.max_lockperiod : 0}&termLength=${this.min_term ? this.min_term : 0},${this.max_term ? this.max_term : 0}&termLengthType=${this.selectedTermLengthType}`
                    if (this.productType.length > 0)
                        this.filtered_data += `&products=${this.productType.join(',')}`
                    this.$emit('apply_filters', this.filtered_data)
                ...
                else if (this.filterType == "featured-products") {
                    this.filtered_data = `rate=${this.min_rate ? this.min_rate : 0},${this.max_rate ? this.max_rate : 0}&lockoutPeriod=${this.min_lockperiod ? this.min_lockperiod : 0},${this.max_lockperiod ? this.max_lockperiod : 0}&termLength=${this.min_term ? this.min_term : 0},${this.max_term ? this.max_term : 0}&termLengthType=${this.selectedTermLengthType}`
                    if (this.productType.length > 0)
                        this.filtered_data += `&products=${this.productType.join(',')}`
                    if (this.fiorganization.length > 0)
        ...
        watch: {
            ...
            min_subscription(newValue, oldValue) {
                this.checkFilters()
            },
            max_subscription(newValue, oldValue) {
                this.checkFilters()
            },

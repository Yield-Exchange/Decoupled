<template>
    <div v-if="variant==='success'" class="timer-clock running">
        <p>{{ formattedTime }}</p>
    </div>
    <div v-else class="timer-clocki runningi">
        <p>{{ formattedTime }}</p>
    </div>
</template>
<style scoped>
    ...
    .timer-clocki.expiring>p {
        color: #FF2E2E;
    }

    .timer-clocki.runningi>p {
        color: #5063F4;
    }
</style>
<script>
    export default {
        props: ['targetTime', 'timezone', 'variant'],
        data() {

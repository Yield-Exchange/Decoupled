                        ...
                        <table class="pr-gic-table w-100">
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    <td class="text-capitalize"> {{ depositReq.compound_frequency.toLowerCase() }} </td>
                                    <td> {{ formatDateToCustomFormat(depositReq.date_of_deposit) }} </td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-3">
                                <CustomDatePicker @hasError="(checkerVariable) => maturityDateError = checkerVariable"
                                    ...
                                    :min="maturityDateMinDate" :max="maturityDateMaxDate" v-model="maturity_date"
                                    :required="true" label="Maturity Date" placeholder="Select  maturity date"
                                    input_type="text" />
                            </div>

                            <div class="col-md-3">
                                <CustomTextInput @hasError="(checkerVariable) => gicNumberError = checkerVariable"
                                    :isemptycheck="requiredChecker" :currentValue="gic_number" v-model="gic_number"
                                    :required="true" label="GIC Number" placeholder="Enter GIC number"
                                    input_type="text" />
                            </div>
...
import CustomDatePicker from '../shared/DatePicker.vue'
...
export default {
    ...
    mounted() {
        ...
        if (this.deposit) {
            ...
            this.current_rate = this.deposit?.offer.interest_rate_offer
        }
    },
    data() {
        return {
    ...
    methods: {
        ...
        formatDateFn(value) {
            return moment(value).format(this.formatDate)
        },
        goBack() {

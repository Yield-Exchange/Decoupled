...
export default {
    ...
    methods: {
        minAmountChange(value, index) {
            ...
            this.min_amount[index] = value
            let min_amount = this.removeCommas(value)
            let max_amount = this.max_amount[index] != null ? this.removeCommas(this.max_amount[index]) : null

            if (min_amount <= this.maxamount && min_amount > 0) {
                ...
                if (min_amount > 0)
                    ...
                    this.$set(this.minAmountError, index, "This field is required")
            }
        },
        maxAmountChange(value, index) {
            // this.min_amount[index] = value
            let min_amount = this.min_amount[index] != null ? this.removeCommas(this.min_amount[index]) : null
            let max_amount = this.removeCommas(value)

            this.max_amount[index] = value
            if (max_amount <= this.maxamount && max_amount > 0) {
                if (min_amount != null && max_amount < min_amount)
                    this.$set(this.maxAmountError, index, "Amount Cannot Be less than minimum amount")
                else {
                    this.$set(this.maxAmountError, index, null)
                    if (min_amount <= this.maxamount)
                        this.$set(this.minAmountError, index, null)
                }
            } else {
                if (max_amount > 0)
                    ...
                    this.$set(this.maxAmountError, index, "This field is required")
            }
        },

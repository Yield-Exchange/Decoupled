    ...
    methods: {
        ...
        search(value) {
            ...
            if (this.filterString !== '') {
                url = `/trade/CG/get-trade-requests?type=inProgress&search=${value}`;
            } else {
                url = `/trade/CG/get-trade-requests?type=inProgress&search=${value}`
            }
            this.getData(url);
        ...
        submitFilters(value) {
            ...
            let url = `/trade/CG/get-trade-requests?type=inProgress&${value}`;
            this.getData(url);
        },
        ...
        getPageData(url) {
            if (this.filterString != '') {
                ...
                url = `${url}&${this.filterString}&type=inProgress&`
            } else {
                url = `${url}&type=inProgress&`
            }
            this.getData(url);
        },
        getData(url) {
            ...
            let this_ = this;
            url = url ? url : "/trade/CG/get-trade-requests?type=inProgress";
            axios.get(url)
                .then(response => {
                    let table_data = [];

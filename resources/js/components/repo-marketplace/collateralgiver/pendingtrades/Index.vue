<template>
    <div style="height: 100%">
        ...
        <div class="ml-10 mt-3" style="width:100%;display: flex; flex-direction:row; justify-content:flex-end;">
            <FilterBox :filtered="filtered" :dontshow="['tradedate']" @apply_filters="submitFilters"
...
export default {
    ...
    data() {
        let columnss = ['Trade ID', 'Settlement Date', 'Collateral Taker', 'Product', 'Collateral', 'Term Length', 'Investment', 'Rate', 'Chat', 'Action'];
        let act = [
            {
                name: "View Offers",
                component: ViewOffer
    ...
    methods: {
        ...
        search(value) {
            ...
            if (this.filterString !== '') {
                url = `/trade/CG/get-deposits?type=pending&search=${value}`;
            } else {
                url = `/trade/CG/get-deposits?type=pending&search=${value}`
            }
            this.getData(url);
        ...
        submitFilters(value) {
            ...
            let url = `/trade/CG/get-deposits?type=pending&${value}`;
            this.getData(url);
        },
        ...
        getData(url) {
            ...
            let this_ = this;
            url = url ? url : "/trade/CG/get-deposits?type=pending";
            axios.get(url)
                .then(response => {
                    let table_data = [];
                    ...
                    Object.values(response?.data.data).forEach((item) => {
                        ...
                        table_data.push([
                            ...
                            item?.settlement_date ? formatTimestamp(item?.c_g_offer.settlement_date, false) : '-',
                            () => {
                            ...
                            () => {
                                return ({ 'component': StartChat, 'attrs': { deposit_id: item.encoded_id, recipient: item?.c_t_organization, reference: item?.deposit_reference_no } });
                            },
                        ]);
                    });

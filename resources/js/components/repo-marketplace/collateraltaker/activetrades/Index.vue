    ...
    <div style="height: 100%">
        ...
        <div class="ml-10 mt-3" style="width:100%;display: flex; flex-direction:row; justify-content:flex-end;">
            <FilterBox :filtered="filtered" @apply_filters="submitFilters" @clear_filters="clearFilters"
                filterType="depositor" :products="products" @searching="search" from="reviewoffers"
                :dontshow="['tradedate']">
            </FilterBox>
        </div>
        <div class="mt-3">
...
import { addDaysToDate, repoProductName, addCommasToANumber, formatTimestamp, formatNumberAbbreviated, sentenceCase, addCommasAndDecToANumber, getBasketDetails, addDaysOrMonthsToDate } from "../../../../utils/commonUtils";
...
export default {
    ...
    data() {
        let columnss = ['Trade ID', 'Collateral Giver', 'Repo Name', 'Collateral', 'Term Length', 'Investment', 'Rate', 'Maturity Date', 'Chat', 'Action'];
        let act = [
            {
                name: "View Offers",
                component: ViewOffer
    ...
    methods: {
        ...
        submitFilters(value) {
            ...
            let url = `/trade/CT/get-deposits?type=active&${value}`;
            this.getData(url);
        },
        search(value) {
            let url = '';
            if (this.filterString !== '') {
                url = `/trade/CT/get-deposits?type=active&search=${value}`;
            } else {
                url = `/trade/CT/get-deposits?type=active&search=${value}`
            }
            this.getData(url);
        ...
        getData(url) {
            ...
            let this_ = this;
            url = url ? url : "/trade/CT/get-deposits?type=active";
            axios.get(url)
                .then(response => {
                    let table_data = [];
                    ...
                    Object.values(response?.data.data).forEach((item) => {
                        ...
                        table_data.push([
                            ...
                            () => {
                                return ({ 'component': StartChat, 'attrs': { deposit_id: item.encoded_id, reference: item?.deposit_reference_no, recipient: item?.c_g_offer?.invitee?.organization } });
                            },
                        ]);
                    });

...
export default {
    ...
    data() {
        let columnss = ['Trade ID', 'Settlement Date', 'Collateral Giver', 'Product', 'Collateral', 'Term Length', 'Investment', 'Rate', 'Chat', 'Action'];
        let act = [
            {
                name: "View Offers",
                component: ViewOffer
    ...
    methods: {
        ...
        submitFilters(value) {
            ...
            let url = `/trade/CT/get-deposits?type=pending&${value}`;
            this.getData(url);
        },
        search(value) {
            let url = '';
            if (this.filterString !== '') {
                url = `/trade/CT/get-deposits?type=pending&search=${value}`;
            } else {
                url = `/trade/CT/get-deposits?type=pending&search=${value}`
            }
            this.getData(url);
        ...
        getData(url) {
            ...
            let this_ = this;
            url = url ? url : "/trade/CT/get-deposits?type=pending";
                ...
                .then(response => {
                    ...
                    Object.values(response?.data.data).forEach((item) => {
                        ...
                        table_data.push([
                            ...
                            () => {
                                return ({ 'component': StartChat, 'attrs': { reference: item?.deposit_reference_no, deposit_id: item.encoded_id, recipient: item?.c_g_offer?.invitee?.organization } });
                            },

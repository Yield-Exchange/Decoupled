        ...
        <div class="row">
            <div class="col-xl-12">
                <div class="card">
                        ...
                        <table class="table text-nowrap">
                            <tbody>
                                <tr class="table-border-double">
                                    <td colspan="3" class="my_h"><span class="b_b"></span>Collaterals<span
                    ...
                    <div class="col-sm-12">
                        ...
                        <div class="table-responsive">
                            <table class="table users-table table-condensed table-hover table-stripe" ref="userTable">
                                ...
                                <tbody>
                                    <tr role="row" v-for="collateral in collaterals">
                                        <td>{{ collateral.id }}</td>
                                        <td>{{ collateral.collateral_name }}</td>
                                        <td> {{ collateral.is_disabled == 1 ? 'Inactive' : 'Active' }}</td>
                                        <td>
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                                    <a class="dropdown-item " href="javascript:void()"
                                                        data-toggle="modal" :data-target="'#edit-' + collateral.id"
                                                        @click="addOrEdit('edit', collateral)">Edit</a>
                                                    <!-- <a class="dropdown-item admin-action-on-admin"
                                                        @click="toggle_state(collateral.encoded_id, collateral.is_disabled)">
                                                        {{ collateral.is_disabled == 1 ? "Activate" : "Deactivate"
                                                        }}
                                                    </a> -->
                                                </div>
                                            </div>
                        ...
                        <div class="bottom-section d-none" v-if="collaterals">
                            <div class="counter">
                                <span>Showing {{ parseInt(startQueryFrom) }} to
                                    {{ startQueryFrom + parseInt(collaterals.length) }} of
                                    {{ totalData }}
                                    entries</span>
                            </div>
                            <div class="paginate">
                                ...
                                <span class="links">
                                    <button v-for="page in this.pages"
                                        :class="{ 'active is_disabled': (currentPage == page), 'is_disabled': (page == '...') }"
                    ...
                    <addCollateralModal v-if="add_product" :product="selected_product" :action="action"
...
import addCollateralModal from './addCollateralModal.vue';
...
export default {
    ...
    data() {
        return {
            ...
            length: [10, 25, 50, 100],
            columns_table_head: ["Collateral Name", "Status"],
    ...
    methods: {
        ...
        async getProductList() {
            await axios.get('/common/trade/get-colletarals-list', response => {
                console.log(response)
            })
        },

        async get_collaterals() {
            let data = new FormData();

            await axios.get('/common/trade/get-colletarals-list')
                .then(response => {
                    ...
                    Swal({
                        title: "Error!",
                        text: "Failed to get Collaterals",
                        icon: "error"
                    });
...
.bottom-section .paginate .links button.active.is_disabled {
    background-color: rgb(184, 219, 249);
    cursor: default;
}

.bottom-section .paginate .links button.is_disabled {
    background-color: rgb(231, 231, 231);
    cursor: default;
}

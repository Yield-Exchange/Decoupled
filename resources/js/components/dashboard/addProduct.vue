    ...
    <Modal :show="show" @isVisible="closeModal" modalsize="lg">
        ...
        descriptionError }}
                ...
                <div class="row">
                    <div class="col-md-12 well">
                        <div class="form-group">
                            <textarea class="w-100 p-2 form-control" rows="6" v-model="product_desc"
        ...
        deactivationDateError }}
                ...
                <div class="row" v-if="action == 'edit' && product.is_disabled == 1">
                    <div class="col-md-12 well">
                        <div class="form-group">
                            <b-form-datepicker :class="(activationDateError ? 'is-invalid' : '')"
                                style="border-radius: 10px;" placeholder="Set Date to activate (optional)"
                                :id="'activationDate' + product.id" :value="product.disabled_until"
                                class="font-13 input-height mb-2 text-center" v-bind="{ min: new Date() }">
                            </b-form-datepicker>
                            <b-alert v-if="activationDateError" show variant="danger" class="form-alert">{{
        activationDateError }}
                            </b-alert>
                        </div>
                    </div>
        ...
        statusError }}
                ...
                <div class="row d-none" v-if="status == 'activate' && action == 'add'">
                    <div class="col-md-12 well">
                        <div class="form-group">
                            <b-form-datepicker :class="(deactivationDateError ? 'is-invalid' : '')"
                                style="border-radius: 10px;" placeholder="Set Date to Deactivate (optional)"
                                v-model="deactivationDate" class="font-13 input-height mb-2 text-center"
                                v-bind="{ min: new Date() }">
                            </b-form-datepicker>
                            <b-alert v-if="deactivationDateError" show variant="danger" class="form-alert">{{
        deactivationDateError }}
                ...
                <div class="row d-none" v-if="status == 'deactivate' && action == 'add'">
                    <div class="col-md-12 well">
                        <div class="form-group">
                            <b-form-datepicker :class="(activationDateError ? 'is-invalid' : '')"
                                style="border-radius: 10px;" placeholder="Set Date to activate (optional)"
                                v-model="activationDate" class="font-13 input-height mb-2 text-center"
                                v-bind="{ min: new Date() }">
                            </b-form-datepicker>
                            <b-alert v-if="activationDateError" show variant="danger" class="form-alert">{{
        activationDateError }}
...
export default {
    ...
    methods: {
        ...
        setDefaults() {
            this.description = this.product.product_name
            this.product_desc = this.product.description
        ...
        createProduct() {
            ...
            let url = null
            if (this.action == 'add')
                url = '/yie-admin/trade/add-new-product'
            else
                url = '/yie-admin/trade/update-product/' + this.product.encoded_id
            axios.post(url, data)
                .then(response => {
                    if (response?.data?.status == false) {
                        Swal({

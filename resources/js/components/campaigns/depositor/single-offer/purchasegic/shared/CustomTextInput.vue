<template>
    <div class="w-100 d-flex flex-column">
        <label class="custom-label" for="">{{ label }} <span v-if="required" class="required">*</span>
        ...
        <input :class="{ 'custom-input-error ': error }" :maxlength="maxLength" v-model="inputValue" :disabled="disabled"
            @keyup="onValueChanged" class="custom-input" :placeholder="placeholder">
...
export default
    {
        props: ['input_type', 'label', 'currentValue', 'placeholder', 'required', 'tooltip', 'tooltiptitle', 'value', 'disabled', 'isemptycheck', 'minlength', 'maxlength'],
        data() {
            return {
                error: false,
                errorMessage: null,
        ...
        methods: {
                ...
                if (this.sanitizeInput(this.inputValue) === null) {
                    ...
                    if (this.inputValue != null) {
                        ...
                        if (this.required && newValue.length < this.minLength) {
                            this.error = true
                            if (this.label.includes("Telephone"))
                                this.errorMessage = `The telephone number is too short`
                            else
                                this.errorMessage = `The ${this.label.toLowerCase()} is too short`
                            ...
                            (newValue.length != 0) ? this.websiteValidation(newValue) : this.error = false;
            ...
            websiteValidation(url) {
                ...
                if (url) {
                    ...
                    if (!t) {
                        if (this.label.toLowerCase().includes("website"))
                            this.errorMessage = `Invalid website address`;
                        else
                            this.errorMessage = `Invalid linkedIn profile address`;

<template>
    <div class="w-100 d-flex flex-column">
        ...
        <MultiSelect :class="{ 'custom-input-error ': error }" class="" :placeholder="placeholder"
            v-model="selectedValue" :options="options" :settings="setting"></MultiSelect>
...
export default
    {
        props: ['input_type', 'label', 'placeholder', 'required', 'isemptycheck', 'currentValue'],
        components: {
            MultiSelect: Select2
        },
        mounted() {
        ...
        data() {
            return {
                options: ['GIC Acquisition', 'Rate Shopping', 'Market Insights'],
                setting: {
                    allowClear: true,
                    multiple: true,
                },
        ...
        watch: {
            selectedValue() {
                this.$emit('customSelect', this.selectedValue)

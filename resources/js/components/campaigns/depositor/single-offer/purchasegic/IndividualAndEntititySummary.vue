<template>
    <div class="w-100 d-flex flex-column justify-content-end">
        <div class="w-100 d-flex flex-column justify-content-end" v-if="currentStep == 'summarysection'">
            ...
            <ActionMessageModal @close="success = false" :show="success" width="600" title="Are you Finished?"
                icon="signup/danger.svg" primarybuttontext="Yes" outlinedbuttontext="No"
                message="Have you added all directors, signing officers, and beneficial owners? "
            ...
            <ActionMessageModal @close="ownershiperror = false" :show="ownershiperror" width="600"
                title="Ownership Breakdown Error" icon="signup/danger.svg" primarybuttontext="" outlinedbuttontext=""
        ...
        <KeyIndividuals :isedit="edituser" @setCurrentStep="(value) => currentStep = value" :userposition="user_pos"
            @stopedit="edituser = false" v-if="currentStep == 'keyIndividuals'" />
        <EntityDetails @setCurrentStep="(value) => currentStep = value" :isedit="editentity"
            :entityposition="entity_pos" @stopedit="editentity = false" v-if="currentStep == 'entitydetails'" />
    </div>
...
import DoubleCheckbox from '../../../../auth/signup/shared/DoubleCheckbox.vue';
...
export default {
    components: {
        CustomSubmit, CustomTextInput, CustomSelectInput, DoubleCheckbox, ActionMessageModal, KeyIndividuals, EntityDetails
    },
    mounted() {
    ...
    methods: {
        ...
        EditEntity(value) {
            ...
            this.currentStep = "entitydetails"
        ...
        async setDefaults() {
            await axios.get(`/organization-data/${this.organization_id}`).then(response => {
                ...
                if (responsedata.key_individuals.length > 0) {
                    let key_individuals = []
                    responsedata.key_individuals.forEach(element => {
                        let individualtemp = this.newIndividual(element)
                        key_individuals.push(individualtemp)
                    });
                    this.$store.dispatch('setkeyIndividuals', key_individuals)
                }
                if (responsedata.entities.length > 0) {
                    let entities = []
                    responsedata.entities.forEach(element => {
        ...
        goBack() {
            this.$emit('goBack', 'individualandentitysummary')
        },
        goNext() {
            this.$emit('updateFactory', 'individualandentitysummary')
        ...
        addEntityToDb() {
            ...
            axios.post('/dep-add-an-entity', { 'entities': this.getOrganizationEntities, 'organization_id': this.organization_id }).then(response => {
        ...
        addKeyIndividualToDb() {
            ...
            axios.post('/dep-add-an-individual', { 'individuals': this.getkeyIndividuals, 'organization_id': this.organization_id }).then(response => {
        ...
        doSubmit() {
            ...
            if (this.getkeyIndividuals.length > 0) {
                const hasDirector = this.getkeyIndividuals.some(element => element.is_director);

                this.getkeyIndividuals.forEach(element => {
                    if (element.is_owenershipabovetwentyfive) {
                        ownnershippercent += parseInt(element.percentage_ownwership)
                    }

                })
                if (ownnershippercent >= 80 && ownnershippercent <= 100 && hasDirector) {
                    this.sending = true
                    this.addKeyIndividualToDb()
                } else {
                    this.ownershiperror = true
                    this.failmessage = "Please provide a breakdown of ownership comprising over 80% of the total shares, including at least one director."
                }
            } else {
                this.ownershiperror = true
                this.failmessage = "Please provide a breakdown of ownership comprising over 80% of the total shares, including at least one director."
        ...
        addIndividual() {
            ...
            this.currentStep = "keyIndividuals"
        },
        EditUser(value) {
            this.edituser = true
            this.user_pos = value
        ...
        deleteUser(index) {
            ...
            if (usertoremove.id == null && usertoremove.user_id == null) {
                ...
                axios.post('/dep-remove-an-individual', { 'id': usertoremove.id, 'organization_id': this.organization_id }).then(response => {
                    if (response.data.success) {
                        response.data.data.forEach(element => {
                            let tempindividual = this.newIndividual(element)
                            individuals.push(tempindividual)
        ...
        deleteEntity(index) {
            ...
            if (entitytoremove.id == null) {
                ...
                axios.post('/dep-remove-an-entity', { 'id': entitytoremove.id, 'organization_id': this.organization_id }).then(response => {
                    if (response.data.success) {
                        response.data.data.forEach(element => {
                            let tempentity = this.newEntity(element)
                            entities.push(tempentity)

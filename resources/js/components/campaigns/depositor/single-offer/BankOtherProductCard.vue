<template>
    <div class="new-offers-tile">
        <div class="frame">
            <div style="text-transform: capitalize" class="non-redeemable">{{ datum?.lockout_period > 0 ? datum?.lockout_period+' '+pluralizeWord('Day',datum?.lockout_period) : '' }} {{ datum?.description }}</div>
            ...
            <div class="frame-wrapper">
                <div class="frame-2">
                    <avatar v-if="!datum?.campaign?.organization?.logo" :size="44" :color="'white'" :backgroundColor="'#4975E3'"
                            :initials="datum?.campaign?.organization?.name[0]"></avatar>
                    <avatar v-else :size="44" :color="'white'" :backgroundColor="'#4975E3'"
                            :src="'/image/'+datum?.campaign?.organization?.logo"></avatar>
                    <div class="text-wrapper-6">{{ addEllipsisIfNecessary(datum?.campaign?.organization?.name,30) }}</div>
                </div>
            </div>
            <div class="div-wrapper">
                <div style="width: 100%;align-self: stretch; padding-top: 10px; padding-bottom: 10px; justify-content: flex-start; align-items: flex-start; gap: 10px; display: inline-flex; cursor: pointer">
                    <div style="flex: 1 1 0; align-self: stretch; justify-content: flex-start; align-items: flex-start; gap: 10px; display: flex">
                        <div @click="viewOffer" style="width:100%;align-self: stretch; height: 40px; border-radius: 20px; overflow: hidden; border: 2px #5063F4 solid; justify-content: center; align-items: center; display: inline-flex">
                            <div style="padding-left: 10px; padding-right: 10px; justify-content: center; align-items: flex-start; gap: 10px; display: flex">
                                <div style="color: #5063F4; font-size: 16px; font-weight: 500; text-transform: capitalize; line-height: 20px; word-wrap: break-word">View More</div>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div style="display: flex; flex-direction: row; justify-content: flex-end; width: 100%;">
            <div class="frame-3"><div class="text-wrapper-7">{{ calculateTimeLeft(datum.expiry_date) }}</div></div>
        </div>
    </div>
</template>
<style scoped>

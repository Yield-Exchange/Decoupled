<template>
    <div>
        ...
        <ConfirmDeletionPrompt :size="successModalSize" @closedSuccessModal="cancelIt" btnOneText="Previous"
            btnTwoText="Activate" :title="`Confirm Activation of ${actionObjectDetails[1]}`" :showm="deleteStep === 1"
            @btnOneClicked="cancelIt" @btnTwoClicked="deleteIt()" message="This operation cannot be undone." />

        <DeleteProductSuccess :size="successModalSize" @closedSuccessModal="cancelIt" :title="successTitle"
            ...
            deleteIt() {
                ...
                axios.post("/campaigns/fi/delete-product", {
                    ...
                    if (response?.data?.success) {
                        ...
                        this.successTitle = response?.data?.message;
                    } else {

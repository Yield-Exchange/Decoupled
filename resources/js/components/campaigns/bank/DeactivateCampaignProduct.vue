<template>
    <div>
        ...
        <ConfirmDeletionPrompt :size="successModalSize" @closedSuccessModal="cancelIt" btnOneText="Cancel"
            btnTwoText="Delete" title='Delete Campaign Product' :showm="deleteStep === 1" @btnOneClicked="cancelIt"
            @btnTwoClicked="deleteIt()" message="This operation cannot be undone." />
        <DeleteProductSuccess :size="successModalSize" @closedSuccessModal="toCampaigns"
            title="Product Removed Successfully." :showm="showSuccessModal" @btnOneClicked="showSuccessModal=false"
            @btnTwoClicked="dismissIt()" />
    </div>
</template>
<script>
    ...
    import DeleteProductSuccess from "../../shared/messageboxes/TwoButtonActionMessageBox.vue";
    ...
    export default {
        ...
        methods: {
            toCampaigns() {
                //alert(this.campaign_id, " this.campaign_id");
                if (this.campaign_deactivate === 0) {
            ...
            deleteIt() {
                ...
                axios.post("/campaigns/fi/my-campaigns/remove-product", {
                    campaign_fi_campaign_product_id: this.actionId
                }).then(response => {

                    if (response?.data?.success) {
                        this.campaign_id = response?.data?.data?.campaign_id;
                        this.campaign_deactivate = response?.data?.deactivated;

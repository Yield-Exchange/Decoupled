    ...
    <div
        ...
        <div style="width: 100%">
            <table class="table table-vue-custom ">
                        ...
                        <th style="padding-left: 10px;">
                            <div style="display: flex; flex-direction: row; gap: 5px">
                                ...
                                <Tooltip placement="top" target="maxnumber"
                                    message="Has to be a number between 0 and 9,999,999,999,999. It should be less than maximum amount." />
                            </div>
                        </th>
                ...
                <tbody>
                    <tr v-if="products_data" v-for="(selected_product, index) in products_data"
                        style="border-bottom: 1 px solid rgb(238, 233, 233);">
                        <td>
                            <div style="margin-top: 20px !important;">
                                {{ capitalize(selected_product.custom_product_name) }}
                            </div>
                        ...
                        <td>
                            ...
                            <CustomInput inputType="number"
                                ...
                                :id="'rate_' + index" name="Rate*" :has-validation="true"
                                @inputChanged="setProductRateOnChange(selected_product.id,$event)" input-type="number"
                                :defaultValue="getProductInfo(selected_product.id).rate"
                                :hasSpecificError="(fieldErrors[`rate_${selected_product.id}`])" />
                            <div class="error-message"
                                v-if="((fieldErrors[`rate_${selected_product.id}`]) ||(fieldErrors[`minimum_${selected_product.id}`]) ||(fieldErrors[`maximum_${selected_product.id}`]))">
                                {{ fieldErrors[`rate_${selected_product.id}`] }}
                            </div>
                        ...
                        <td>
                            ...
                            <CustomInput inputType="number"
                                c-style="font-weight: 400;width:100%; margin-top:0px;height:90%;" p-style="width:100%"
                                :id="'minimum_' + index" name="Minimum*" :has-validation="true"
                                @inputChanged="setProductMinimumOnChange(selected_product.id,$event)"
                                input-type="number"
                                :defaultValue="getProductMinimum(getProductInfo(selected_product.id))"
                                :hasSpecificError="(fieldErrors[`minimum_${selected_product.id}`])" />
                            <div class="error-message"
                                v-if="((fieldErrors[`rate_${selected_product.id}`]) ||(fieldErrors[`minimum_${selected_product.id}`]) ||(fieldErrors[`maximum_${selected_product.id}`]))">
                                {{fieldErrors[`minimum_${selected_product.id}`]}}
                            </div>
                        ...
                        <td>
                            ...
                            <CustomInput inputType="number"
                                ...
                                @inputChanged="setProductMaximumOnChange(selected_product.id,$event)"
                                input-type="number"
                                :defaultValue="getProductMaximum(getProductInfo(selected_product.id))"
                                :hasSpecificError="(fieldErrors[`maximum_${selected_product.id}`])" />
                            <div class="error-message"
                                v-if="((fieldErrors[`rate_${selected_product.id}`]) ||(fieldErrors[`minimum_${selected_product.id}`]) ||(fieldErrors[`maximum_${selected_product.id}`]))">
                                {{ fieldErrors[`maximum_${selected_product.id}`] }}
                            </div>

                        </td>
                        <!-- <td>
                            <CustomInput c-style="font-weight: 400;width:100%;" p-style="width:100%" id="rate"
                                name="Order Limit*" :has-validation="true"
                                @inputChanged="order_limit[selected_product.id] = $event" input-type="number"
                                :defaultValue="getProductOrderLimit(getProductInfo(selected_product.id))"
                                :hasSpecificError="(fieldErrors[`order_limit_${selected_product.id}`])" /> -->
                        <!-- <div class="error-message" v-if="fieldErrors[`order_limit_${index}`]">
                                    {{ fieldErrors[`order_limit_${index}`] }}
                                    </div> -->
                        <!-- </td> -->
                        <td>
        ...
        <!-- <TwoButtonActionMessageBox2 size="md" @closedSuccessModal="goingPrevious = false" btnTwoText="Yes" btnOneText="No"
            :title="confirmdraftmsg" :showm="goingPrevious" @btnOneClicked="handleProceed"
            @btnTwoClicked="handleSaveAndProceed" /> -->

        <OKButtonActionMessageBox :size="successModalSize" :title="successModalTitle"
            @closedSuccessModal="closeSuccessModal" :showm="showSuccessModal" @okClicked="closeSuccessModal" />
...
<script>
    ...
    export default {
        mounted() {
            this.getMyProducts();
            // console.log(this.data?.campaign_products, " Edit products Rate ")
        },
        computed: {
        ...
        props: ['selected_products', 'data', 'collected_details_data', 'selected_groups'],
        data() {
        ...
        methods: {
            ...
            previousBtn() {
                ...
                this.$emit('back-step', 3);
                // if () {
                //     this.saveAsDraft(false, true);
                // } else {
                //     this.goingPrevious = true;
                //     let products__ = this.selected_products;
                //     console.log(products__, "products__");
                //     console.log(this.goingPrevious, "ev products__");
            ...
            handleSaveAndProceed() {
                ...
                let products__ = this.saveAsDraft(true, true);
                if (products__) {
            ...
            getProductInfo(product_id, place_holder = true, field = "") {
                ...
                if (this.selected_products) {
                    d = this.selected_products.filter(e => {
                    ...
                    d = this.data.campaign_products.filter(e => {
                        let p = e?.fi_campaign_product_id;
                        if (p === undefined) {
                            p = e?.product_id;
                        }
            ...
            submitRates() {
                ...
                // console.log(products__, "prodcts")
                if (products__) {
                    ...
                    axios.post('/campaigns/fi/update-campaign', {
                        ...
                        .then(response => {
                            ...
                            if (response?.data?.success) {
                                this.successModalTitle = "Rates Updated Successfully"
            ...
            getMyProducts() {
                ...
                this.getCampaignSelectedProducts.forEach(e => {
                    ...
                    prod_ids.push(p);
                    this.minimum[p] = e?.minimum;
                    this.maximum[p] = e?.maximum;
                    this.rate[p] = e?.rate;
                });
            ...
            saveAsDraft(isNext = false, isPreviousBTN = false, novalidation = false) {
                    ...
                    this.selected_products.forEach((e1, index1) => {
                        ...
                        if (p) {
                            if (novalidation) {
                                ...
                                if (this.maximum[p] === null || this.maximum[p] === "" || this.maximum[p] === undefined || isNaN(this.sanitizeAmount(this.maximum[p]))) {
                                    has_empty_product_ = true;
                                    this.$set(this.fieldErrors, `maximum_${p}`, 'Enter valid maximum amount.');
                                }
                                else {
                                    if ((this.sanitizeAmount(this.maximum[p]) < 1) || (this.sanitizeAmount(this.maximum[p]) > 999999999999)) {
                                        has_empty_product_ = true;
                                        this.$set(this.fieldErrors, `maximum_${p}`, 'Enter Valid maximum Amount .');
                                    } else {

                                        if ((this.sanitizeAmount(this.minimum[p]) !== 0) && (this.sanitizeAmount(this.maximum[p]) < this.sanitizeAmount(this.minimum[p]))) {
                                            has_empty_product_ = true;
                                            this.$set(this.fieldErrors, `minimum_${p}`, 'Cannot be greater than maximum');
                                            this.$set(this.fieldErrors, `maximum_${p}`, 'Cannot be less than minimum');
                                        } else {
                                            this.$set(this.fieldErrors, `minimum_${p}`, '');
                                            this.$set(this.fieldErrors, `maximum_${p}`, '');
                                        }
                                ...
                                if (this.minimum[p] === null || this.minimum[p] === "" || this.minimum[p] === undefined || isNaN(this.sanitizeAmount(this.minimum[p]))) {
                                    has_empty_product_ = true;
                                    this.$set(this.fieldErrors, `minimum_${p}`, 'Enter valid Minimum amount.');
                                }
                                else {
                                    if ((this.sanitizeAmount(this.minimum[p]) < 1) || (this.sanitizeAmount(this.minimum[p]) > 999999999999)) {
                                        has_empty_product_ = true;
                                        this.$set(this.fieldErrors, `minimum_${p}`, 'Enter Valid Minimum Amount .');
                                    } else {

                                        if ((this.sanitizeAmount(this.maximum[p]) !== 0) && (this.sanitizeAmount(this.minimum[p]) > this.sanitizeAmount(this.maximum[p]))) {
                                            has_empty_product_ = true;
                                            this.$set(this.fieldErrors, `minimum_${p}`, 'Cannot be greater than maximum');
                                            this.$set(this.fieldErrors, `maximum_${p}`, 'Cannot be less than minimum');
                                        } else {
                                ...
                                if (this.rate[p] === null || this.rate[p] === "" || isNaN(this.rate[p]) || this.rate[p] === undefined) {
                                    has_empty_product_ = true;
                                    this.$set(this.fieldErrors, `rate_${p}`, 'Enter a rate value between 1 and 100.');
                                } else {
                                    if ((this.sanitizeAmount(this.rate[p]) < 1) || (this.sanitizeAmount(this.rate[p]) > 100)) {
                                        has_empty_product_ = true;
                                        this.$set(this.fieldErrors, `rate_${p}`, 'Enter a rate value between 1 and 100.');
                                    } else {
                                ...
                                // if (this.order_limit[p] === undefined || this.order_limit[p] === "" || this.order_limit[p] === null) {
                                //     this.$set(this.fieldErrors, `order_limit_${p}`, '');
                                // has_empty_product_ = true;
                                // this.$set(this.fieldErrors, `order_limit_${p}`, 'Please order limit');
                                // } else {
                                //     this.$set(this.fieldErrors, `order_limit_${p}`, '');

                                // if ((!isNaN(this.sanitizeAmount(this.maximum[p])))) {
                                //     if (this.sanitizeAmount(this.order_limit[p]) > this.sanitizeAmount(this.maximum[p])) {
                                //         has_empty_product_ = true;
                                //         this.$set(this.fieldErrors, `order_limit_${p}`, 'This limit amount is greater than the maximum amount of ' + this.maximum[p]);
                                //     }
                                // } else {
                                //     this.$set(this.fieldErrors, `order_limit_${p}`, '');
                                // }
                                ...
                                if (this.rate[p] === undefined || this.rate[p] == "" || this.rate[p] == null) {
                                    ...
                                    if ((!isNaN(this.sanitizeAmount(this.rate[p])))) {
                                        if ((this.sanitizeAmount(this.rate[p]) > 100) || (this.sanitizeAmount(this.rate[p]) < 0)) {
                                            has_empty_product_ = true;
                                            this.$set(this.fieldErrors, `rate_${p}`, 'Your rate has to be between 0 and 100');
                                        }
                                    } else {
                                        this.$set(this.fieldErrors, `rate_${p}`, '');
                                    }
                ...
                axios.post(campaign_id ? '/campaigns/fi/update-campaign' : '/campaigns/fi/create-campaign', {
                    ...
                    'status': 'ACTIVE',
                    'products': products_,
                    'campaign': (this.forcedDraftEdit) ? this.campaignId : campaign_id,
                    'groups': groups_
                })
                    .then(response => {
                        this.submittedSaveDraft = false;
                        if (response?.data?.success) {
                            ...
                            this.isDraftAlready = true;
                            this.successModalTitle = "Saved Successfully to Drafts.";
                            // this.successMessage="Campaign details saved successfully to drafts."
                            this.showSuccessModal = true;
                            this.successbtnOneText = "Previous Step";
                            this.successbtnTwoText = "Next Step";
                        } else {
                            throw new Error(response?.data?.message);
                        }
                    }).catch(error => {

...
@section('page_content')
    ...
    <div class="row">
        <div class="col-xl-12">
                ...
                <div class="table-responsive">
                    <table class="table text-nowrap">
                        ...
                        <tr class="table-active table-border-double">
                            <td colspan="3">
                                <span class="b_b">BLOGS </span>TAGS<span class="badge bg-blue badge-pill total_records_pill"></span>
                            </td>
                            <td class="text-right">
                                @if($user->userCan('admin/blogs-tags/page-access'))
                                <a data-toggle="modal" data-target="#create_blog_tag" href="javascript:void()" class="btn btn-primary mmy_btn pull-right"><i class="fa fa-plus"></i> Create Tag</a>
                                @endif
                            </td>
                        </tr>
                        </tbody>
            ...
            <div id="create_blog_tag" class="modal fade" role="dialog">
                <div class="modal-dialog  modal-sm">
                    <!-- Modal content-->
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title"><b>Add New Blog Tag</b></h5>
                                ...
                                <div class="row">
                                    <div class="col-md-12 well">
                                        ...
                                        <div class="form-group">
                                            <input type="text" id="name" name="name" placeholder="Enter Tag" maxlength="26" minlength="1"  class="form-control col-md-12" required/>
                                        </div>
                                    </div>
                                </div>
                                <div class="row">
...
@section('scripts')
    <script>
        $(document).on("click",".admin-delete-blog-tag",function (){

            swal({
                title: "",
                text: "Are you sure to delete this blog tag?",
                // icon: "warning",
                buttons: ["No", "Yes"],
            }).then((response) => {
                if(response) {
        ...
        $(document).ready(function () {
            ...
            $('.custom-data-tables').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{route('admin.blogs-tag.data')}}",
                    method:"POST",
                    data: function (d) {
                        d._token= "{{ csrf_token() }}";
                    }
       ...
       <script>
        $(document).on("submit","#CreateCategory",function () {
            ...
            makeApiCall("{{ route('blogs-tag.store') }}", $this_.serialize(), function(response) {
                if(response.success){
                    $(".modal").modal('hide');

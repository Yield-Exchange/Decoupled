...
{{-- <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#leavePageModal">
    Launch demo modal
</button> --}}

<!-- Modal -->
<div class="modal fade show" data-backdrop="static" id="leavePageModal" tabindex="-1" role="dialog"
    aria-labelledby="leavePageModalTitle" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                ...
                <button type="button" id="LeaveNowModal"
...
<script type="text/javascript">
    $(document).ready(function() {
        var anchor = null
        const protected_routes = ['/repos/post-request', '/repos/repos-reviews/summary/*', '/post-request',
            '/request-summary', '/depositor/create-gic/*', '/campaigns',
            '/repos/view-trade-request-summary/*', '/repos/create-basket', '/repos/create-basket/*',
            '/repos/publish-rates-offers','/repos/post-request-with-ai'
        ]
        ...
        const isProtectedRoute = (pathname, routes) => {
            return routes.some(route => {
                // Replace '*' with '.*' to match any characters in that segment
                const regexPattern = route.replace(/\*/g, '.*');
                const regex = new RegExp(`^${regexPattern}$`); // Anchor to match the whole pathname
                return regex.test(pathname);
            });
        };

        const isProtected =(pathname==="#")? false: isProtectedRoute(pathname, protected_routes);
        // console.log(isProtected, "Check if it's protected");

        $(document).on('click', 'a', function(e) {
            if (!$(this).hasClass("paginate_button") && !$(this).hasClass("no-page-exit-alert")) {
                // console.log(protected_routes.includes(pathname), "Check if its prtected")
                if (isProtected) {
                    e.preventDefault();
                    let $this = $(this);
                    let $loader = $("#cover-spin");

                    anchor = e.target.closest("a"); // Find closest Anchor (or self)

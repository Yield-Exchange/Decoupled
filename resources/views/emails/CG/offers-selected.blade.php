...
<div style="padding: 0 5%;margin-right:auto;margin-left:auto" class="responsive">
    ...
    <div class="w-100 d-flex justify-content-center my-2 campaign-status-text "
        ...
        width="100%">
        {{ $selectedOffers['CTOrg'] }} has selected the following Trade offer(s).
    </div>
    ...
    <div class="w-100  " style="width:100%" width="100%">
            ...
            <tbody style="background-color:#F4F5F6; margin-top:30px" bgcolor="#F4F5F6">
                ...
                <tr>
                    ...
                    <td style="padding:0.6rem; text-align:start; color:#252525; font-family:Montserrat; font-size:16px; font-weight:700; word-wrap:break-word; border-bottom:3px solid white; border-top:none"
                        align="start">
                        <?php 
                        if($offer->biColleteral->is_dummy==1){
                            $thereIsdummy=true;
                            echo "Dummy";
                            }else{
                            echo $offer->biColleteral->encoded_cusip_id ;
                            }
                        ?>
                    ...
                    <td style="padding:0.6rem; text-align:start; color:#252525; font-family:Montserrat; font-size:16px; font-weight:700; word-wrap:break-word; border-bottom:3px solid white; border-top:none"
                        align="start">
                        <?php 
                        if($offer->basket->is_dummy==1){
                            $thereIsdummy=true;
                            echo "Dummy";
                            }else{
                            echo $offer->basket->encoded_basket_id;
                            }
                        ?>
        ...
        {{-- <div style="width: 100%; height: 100%; margin: 10px; background: rgba(244, 182, 60, 0.10); justify-content: flex-start; align-items: center; gap: 10px; display: inline-flex">
          
            <div style="flex: 1 1 0; height: 32px; justify-content: flex-start; align-items: center; gap: 10px; display: flex"> --}}
                @if($thereIsdummy)
                <div style="display:flex; flex-direction:row; gap:0px; color: #252525; margin-top:1rem; background: rgba(244, 182, 60, 0.10);  font-size: 14px; font-family: Montserrat; font-weight: 400; line-height: 16px; word-wrap: break-word">
                    <img src="{{ asset('assets/emails/ic_twotone-info.svg') }}"  alt=""
                    >
                   <p> Please update the dummy basket for these offers to the provided basket ID’s to complete your trade by the settlement date. </p>
                </div>
                @endif
            {{-- </div>
        </div> --}}

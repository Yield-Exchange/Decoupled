    ...
    <div style="padding: 0 5%; margin-right:auto; margin-left:auto;" class="responsive">
        ...
        <div class="w-100 d-flex justify-content-center my-2 campaign-status-text "
            ...
            <div class="w-100 d-flex justify-content-center my-2 campaign-status-text "
                ...
                align="center">
                @if ($user_type == 'BANK')
                    You've got a counter offer!
                @elseif ($user_type == 'DEPOSITOR')
                    Counter offer sent!
                @else
                ...
                <div class="w-100  " style="width:100%" width="100%">
                    <table class="border-collapse :collapse;margin:0 auto !important; custom-table w-100 table table-hover"
                        ...
                        <tbody
                            ...
                            @if ($original_offer->interest_rate_offer != $counter_offer->interest_rate_offer)
                                <tr>
                                    ...
                                    <td style="padding:0.6rem; text-align:start; border-bottom:3px solid white; border-top:none; font-size:28px"
                                        align="start">
                                        {{ number_format($original_offer->interest_rate_offer, 2) }}%</td>
                                    <td style="padding:0.6rem; text-align:start; border-bottom:3px solid white; border-top:none; font-size:28px; font-weight:700; color:#252525"
                                        align="start">
                                        {{ number_format($counter_offer->offered_interest_rate, 2) }}%</td>
                                </tr>
                            @endif

                            {{-- minimum amount   --}}
                            @if ($original_offer->minimum_amount != $counter_offer->minimum_amount)
                                <tr>
                                    <td style="padding:0.6rem; text-align:start; border-bottom:3px solid white; border-top:none; color:#5063F4; font-size:28px; font-weight:400"
                                        align="start">Minimum Amount</td>
                                    <td style="padding:0.6rem; text-align:start; border-bottom:3px solid white; border-top:none; font-size:28px"
                                        align="start">
                                        {{ $original_offer->invited->depositRequest->currency }}
                                        {{ number_format(floatval($original_offer->minimum_amount)) }}</td>
                                    <td style="padding:0.6rem; text-align:start; border-bottom:3px solid white; border-top:none; font-size:28px; font-weight:700; color:#252525"
                                        align="start">{{ $original_offer->invited->depositRequest->currency }}
                                        {{ number_format(floatval($counter_offer->minimum_amount)) }}</td>
                                </tr>
                            @endif
                            {{-- maximum amount --}}
                            @if ($original_offer->maximum_amount != $counter_offer->maximum_amount)
                                <tr>
                                    <td style="padding:0.6rem; text-align:start; border-bottom:3px solid white; border-top:none; color:#5063F4; font-size:28px; font-weight:400"
                                    ...
                                    <td style="padding:0.6rem; text-align:start; border-bottom:3px solid white; border-top:none; font-size:28px"
                                        align="start">
                                        {{ $original_offer->invited->depositRequest->currency }}
                                        {{ number_format($original_offer->maximum_amount) }}</td>
                                    <td style="padding:0.6rem; text-align:start; border-bottom:3px solid white; border-top:none; font-size:28px; font-weight:700; color:#252525"
                                        align="start">{{ $original_offer->invited->depositRequest->currency }}
                                        {{ number_format($counter_offer->maximum_amount) }}</td>
                                </tr>
                            @endif
                        </tbody>
                    </table>
                </div>
                <p style="color: #252525;text-align: center;font-family: Montserrat;font-size: 16px;font-style: normal;font-weight: 300;line-height: normal;">
                    @if ($user_type == 'BANK')
                    Respond to this counter by {{$timezoner}} You can either accept, counter, or decline this offer.
                    @elseif($user_type != 'DEPOSITOR' && $user_type != 'BANK')
                    *Counter offer expiry {{$timezoner}}
                    @endif
                </p>

    ...
    <div class="w-100 d-flex justify-content-center my-2 campaign-status-text "
        ...
        <div style="width: 100%; text-align: center;">
            ...
            <div class="w-100 d-flex justify-content-center" style="width:100%" width="100%">
                    ...
                    $invitedorgs=[];
                    @endphp                                
                    @foreach ($CTRequest['orgs'] as $org )
                        @php
                            array_push($invitedorgs,$org->name)
                        @endphp
                    @endforeach 
                    <p class="notify"
                    ...
                    Hello Admin,  <span style="color: #252525; font-size: 24px; font-family: Montserrat; font-weight: 400; word-wrap: break-word"><span style="color: #252525; font-size: 24px; font-family: Montserrat; font-weight: 700; word-wrap: break-word">{{$CTRequest['sender']->name}} </span></span> has sent a request for the following investments to the following institutions:<span style="color: #5063F4; font-size: 24px; font-family: Montserrat; font-weight: 700; word-wrap: break-word"> {{implode(",",$invitedorgs)}} </span>
                     </p>
            </div>
                ...
                <table class="width:100%; border-collapse :collapse;margin:0 auto !important; custom-table w-100 table table-hover"
                    ...
                    <thead style="color:#5063F4; font-weight:700; padding:1rem; background-color:#EFF2FE"
                        ...
                        <tr>
                            <th style="padding:0.6rem; text-align:start; border-bottom:3px solid white; border-top:none"
                                align="start">Preffered Collaterals
                            </th>  
                            <th style="padding:0.6rem; text-align:start; border-bottom:3px solid white; border-top:none"
                            align="start">Term
                        </th> 
                    ...
                    <tbody
                        ...
                        {{-- //{{json_encode($newpostrequestdetails)}} --}}
                         @foreach ($CTRequest['requests'] as $req )
                         <tr>
                            <td style="padding:0.6rem; text-align:start; border-bottom:3px solid white; border-top:none; max-width:200px!important;"
                                align="start">
                                @php
                                $preferedcols=[];
                                @endphp                                
                                @foreach ($req->preferred_collaterals as $preferredcol )
                                    @php
                                        array_push( $preferedcols,$preferredcol->collateral_name)
                                    @endphp
                                @endforeach                               
                                @if(sizeOf($preferedcols)>0)
                                {{implode(",",$preferedcols)}} 
                                @else
                                No Preferred Collaterals.
                                @endif                      
                            </td> 
                                <td style="padding:0.6rem; text-align:start; border-bottom:3px solid white; border-top:none"
                                align="start">
                                {{$req->term_length }} {{$req->term_length_type}}
                            </td> 
                                <td style="padding:0.6rem; text-align:start; border-bottom:3px solid white; border-top:none"
                                align="start">
                                 {{$req->currency}} {{  number_format($req->investment_amount)}}
                                </td>                          
                        </tr>    
                         @endforeach
                    </tbody>
